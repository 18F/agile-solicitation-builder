...
content_components = [
    ...
    {
        "name": "naicsText",
        "text": "This requirement will be solicited under the following North American Industrial Classification System (NAICS) Code: 541512, Computer Systems Design Services. This {DOC_TYPE} will be awarded under FAR 8.4 which governs orders placed under Federal Supply Schedules contracts.",
        "section": 2,
    },
    {
    ...
    {
        "name": "basePeriodDurationNumber",
        "text": "6",
        "section": 2,
    },
    {
        "name": "basePeriodDurationUnit",
        "text": "months",
        "section": 2,
    },
    {
    ...
    {
        "name": "optionPeriodDurationNumber",
        "text": "6",
        "section": 2,
    },
    {
        "name": "optionPeriodDurationUnit",
        "text": "months",
        "section": 2,
    },
    {
    ...
    {
        "name": "iterationPoPNumber",
        "text": "2",
        "section": 2,
    },
    {
        "name": "iterationPoPUnit",
        "text": "weeks",
        "section": 2,
    },
    {
    ...
    {
        "name": "offSiteDevelopmentCompliance",
...
deliverables = [
    ...
    },
    {
        ...
        "text": "The vendor shall provide a solution that can write automated scripts that can verify thousands of scenarios in minutes and then deploy updated code into production environments multiple times a day. Moreover the solution shall use automated performance tests which simulate surges in traffic to identify performance bottlenecks. While manual tests and quality assurance are still necessary, automated tests must be implemented to provide consistent and reliable protection against unintentional regressions, and make it possible for developers to confidently release frequent updates to the service.\n\nHigh level requirements of automated tests include:\n\n  1. Create automated tests that verify all user-facing functionality\n  2. Create unit and integration tests to verify modules and components\n  3. Run tests automatically as part of the build process\n  5. Conduct load and performance tests at regular intervals, including before public launch\n\nAdditionally, it is recommended that vendors follow 18F's automated testing roadmap. Though these steps are not split into 'levels', they should be followed in roughly the order listed below.\n\n  - **Set up a continuous build**: A continuous build system is the first important step in establishing an automated testing culture. Even if the system does not currently have tests, a continuous integration and build system will encourage the team to keep the build free of non test-related breakages.\n  - **Register a coverage bundle**: Establishing baseline code coverage metrics gives visibility into where tests are currently lacking, and encourages progress in those areas.\n  - **Classify tests by size**: Classifying tests by Small (unit), Medium (integration), and Large (system) test sizes identifies the current balance of automated tests.\n  - **Create a smoke test suite**: If a test suite takes too long to run, identify a set of 'smoke tests' that must be run before each submit.\n  - **Identify nondeterministic tests**: Tests that arbitrarily fail without changes to the code that is being tested or the inputs are called 'nondeterministic' or 'flaky'. Flakiness indicates that not all of the inputs to a test are properly controlled, and rely on 'outside resources' such as the system clock, remote systems, etc. As a first step to tackling flaky tests, triage them using a special label or directory, so that team members know that such tests need careful attention and should be fixed eventually.\n  - **Establish Test classification ratio**: For systems beyond a certain size and complexity, there should be a reasonable balance between Small (unit), Medium (integration), and Large (system) tests that makes sense. This enables informed decisions about the kind of tests the system should have and the kind of refactoring work necessary to support this target balance.\n  - **No releases with broken tests**: Once the team has established its continuous build, identified tests by size and flakiness, and possibly defined a smoke test suite, team policy should mandate that all tests must pass before each new release.\n  - **Require tests for all nontrivial changes**: To begin building up coverage, team policy should mandate that all new nontrivial changes should be accompanied by automated tests. If a change is covered by an existing test, it's a judgment call on behalf of the author and reviewer whether or not a new test should be written to test the new change at a different scope from the existing test.\n  - **Require tests or smoke tests to pass before submit**: To ensure the build always remains in a passing state, team policy should dictate that either all tests, or the smoke test suite, should pass before each submitted code change.\n  - **No nondeterministic tests**: Eventually, all flaky tests should be fixed so that they no longer fail barring a change in the test inputs or the code being tested.\n  - **Total coverage of at least X%**: The team should establish a baseline coverage goal and ensure that coverage remains above that baseline level. If the project is free-standing and all the code is new this number should be in the high 90s. If the vendor is working with legacy software such a high number may not be realistic and should be determined relative to the existing coverage. Ex: if existing coverage is 20% the vendor's nitial target coverage could be 35%, or an improvement of 15%.\n  - **All tests and smoke tests run in X minutes or less**: The team should monitor the running time of its entire test suite, or at least the smoke test suite, and ensure that it doesn't cross an agreed-upon upper bound. Similar to the previous goal of establishing a test coverage percent, the target time will depend on how much of the code is new or legacy, and the size and scope of the application itself.",
